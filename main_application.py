...
from files_view import FilesView # Import the new files view
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.sidebar_frame.grid_rowconfigure(7, weight=1) # Adjusted row configure for new button
        ...
        self.scheduling_button = ctk.CTkButton(self.sidebar_frame, text="CPU Scheduling", command=lambda: self.switch_view("scheduling"))
        ...
        self.prediction_button.grid(row=5, column=0, padx=20, pady=10)

        self.files_button = ctk.CTkButton(self.sidebar_frame, text="Project Files", command=lambda: self.switch_view("files"))
        self.files_button.grid(row=6, column=0, padx=20, pady=10)
        ...
        for name, View in {
            ...
            "files": FilesView # Add the new files view
        }.items():
