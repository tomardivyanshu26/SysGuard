...
class FcfsView(ctk.CTkFrame):
    """
    CPU Scheduling view for animating the FCFS algorithm on live processes,
    ...
    def __init__(self, parent):
        ...
        # Title
        title_label = ctk.CTkLabel(self, text=f"CPU Scheduling: FCFS on Live Processes", font=ctk.CTkFont(size=24, weight="bold"))
    ...
    def get_top_cpu_processes_fast(self, num_processes=5):
        """Fetches top CPU processes quickly."""
        procs = []
    ...
    def setup_simulation(self):
        """Initializes simulation with live data, adding simulated arrival times."""
        ...
        for i, proc_data in enumerate(top_processes):
            ...
            burst_time = max(2, int(cpu_usage * 0.1) + 2) # Shorter burst for faster sim
            try:
                p_name = proc.name()
                p_pid = proc.pid
            ...
            self.processes.append({
                "id": p_name, "pid": p_pid, "cpu_usage": cpu_usage, 
                "arrival": i * 2, # Staggered arrival times
    ...
    def render_processes(self):
        ...
        headers = ["Process", "PID", "CPU %", "Arrival", "Burst", "Remaining"]
        ...
        for i, p in enumerate(self.processes):
            ...
            ctk.CTkLabel(self.process_frame, text=p["arrival"]).grid(row=i + 1, column=3)
            ctk.CTkLabel(self.process_frame, text=p["burst"]).grid(row=i + 1, column=4)
            ctk.CTkLabel(self.process_frame, text=p["remaining"]).grid(row=i + 1, column=5)
    ...
    def render_gantt(self):
        ...
        x_pos = 10
        scale = 20 # Smaller scale for potentially longer bursts
    ...
    def tick(self):
        ...
        for p in self.processes:
            if not p["has_arrived"] and self.timer >= p["arrival"]:
                p["has_arrived"] = True
                self.ready_queue.append(p)
        
        # 2. Check if CPU is idle and ready queue has processes
        if not self.current_process and self.ready_queue:
            self.current_process = self.ready_queue.pop(0) # FCFS logic
            self.current_process["start_time"] = self.timer # Mark when it started
        ...
        if self.current_process:
            ...
            if self.current_process["remaining"] == 0:
                self.gantt_data.append({
                    ...
                    "start": self.current_process["start_time"], 
                    "duration": self.current_process["burst"], 
        ...
        if any(p["remaining"] > 0 for p in self.processes) or self.ready_queue or self.current_process:
            ...
            self.render_gantt() # Final render
